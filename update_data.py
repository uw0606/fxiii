...
# 点滅防止用：前回のデータハッシュを保持
last_hashes = {}
last_summary_hashes = {"trends": "", "levels": ""}
...
def get_filtered_economic_events():
    ...
    try:
        ...
        for item in res.data:
            if item.get('currency') in ['USD', 'JPY'] and item.get('importance', 0) >= 2:
                time_str = item.get('event_time')
...
while True:
    ...
    # 概要ファイルもハッシュチェックを行い無駄な書き込みを削減
    trends_json = json.dumps(all_trends, sort_keys=True)
    trends_hash = hashlib.md5(trends_json.encode()).hexdigest()
    if last_summary_hashes["trends"] != trends_hash:
        with open(os.path.join(OUTPUT_DIR, 'all_trends.json'), 'w') as f: 
            f.write(trends_json)
        last_summary_hashes["trends"] = trends_hash

    levels_json = json.dumps(all_levels, sort_keys=True)
    levels_hash = hashlib.md5(levels_json.encode()).hexdigest()
    if last_summary_hashes["levels"] != levels_hash:
        with open(os.path.join(OUTPUT_DIR, 'all_levels.json'), 'w') as f: 
            f.write(levels_json)
        last_summary_hashes["levels"] = levels_hash
